...
static inline tStereo_cross order_1(tStereo_cross *in, coef_1_ord *coeffs, tStereo_cross *delay)
{
    /*float xh = 0.0;
    tStereo_cross out;
    ...
    tStereo_cross xh =  { 0.0, 0.0 };
    tStereo_cross out = { 0.0, 0.0 };

    xh.L = in->L - (coeffs->k0 * delay->L);
    out.L = (coeffs->k0 * xh.L) + delay->L;
    delay->L = xh.L;

    xh.R = in->R - (coeffs->k0 * delay->R);
    out.R = (coeffs->k0 * xh.R) + delay->R;
    delay->R = xh.R;
...
static inline tStereo_cross order_2(tStereo_cross *in, coef_2_ord *c, states_2* s)
{
    tStereo_cross wn;
    tStereo_cross out;
